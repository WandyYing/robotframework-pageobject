...
class FetchYaml(object):
    ...
    def load_yaml(self, path):
        print("pappa: ", path)
        with open(path, 'r') as yml_src:
            return self._smart_load(yml_src.read())

    def get_dictionary(self, yaml):
        return yaml
    ...
    def _get_tree_by_smart_path(self, dct, key):
        ...
        while len(sp) == 3:
            ...
            for i, block in search_pairs:
                ...
                logger.debug(
                    "_smart_path: stepped into branch search path '%s' and got its leaf value: %s" % (path, str(v)))
                if not v: continue
